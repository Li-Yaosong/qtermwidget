...
void TerminalDisplay::setBackgroundColor(const QColor &color) {
    ...
    QPalette scrollBarPalette = _scrollBar->palette();
    QPalette p = palette();
    p.setColor(backgroundRole(), color);
    setPalette(p);

    // Avoid propagating the palette change to the scroll bar
    _scrollBar->setPalette(scrollBarPalette);
    QString style_sheet = qApp->styleSheet();
    _scrollBar->setStyleSheet(scrollBarStyleSheet().arg(_colorTable[DEFAULT_BACK_COLOR].color.name(),
                                                        _colorTable[DEFAULT_FORE_COLOR].color.name()));
...
void TerminalDisplay::paintEvent(QPaintEvent *pe) {
    ...
    QPixmap currentBackgroundImage = _backgroundMovie ? _backgroundMovie->currentPixmap() : _backgroundImage;
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    if (_backgroundVideoPlayer->isPlaying()) {
        currentBackgroundImage = _backgroundVideoFrame;
    }
    ...
    if (!currentBackgroundImage.isNull()) {
        ...
        const QRect r = currentBackgroundImage.rect();
        qreal wRatio = static_cast<qreal>(cr.width()) / r.width();
        qreal hRatio = static_cast<qreal>(cr.height()) / r.height();
        qreal maxScale = qMax(wRatio, hRatio);
        qreal minScale = qMin(wRatio, hRatio);
        if (_backgroundMode == Stretch) { // scale the image without keeping its
                                          // proportions to fill the screen
            paint.drawPixmap(cr, currentBackgroundImage, r);
        } else if (_backgroundMode == Zoom) { // zoom in/out the image to fit it
            QRect tepRect(QPoint(0, 0), r.size() * maxScale);
            tepRect.moveCenter(cr.center());
            paint.drawPixmap(tepRect, currentBackgroundImage, r);
        } else if (_backgroundMode == Fit) { // if the image is bigger than the
                                             // terminal, zoom it out to fit it
            QRect tepRect = minScale < 1.0 ? QRect{QPoint(0, 0), r.size() * minScale} : r;
            tepRect.moveCenter(cr.center());
            paint.drawPixmap(tepRect, currentBackgroundImage, r);
        } else if (_backgroundMode == Center) { // center the image without scaling/zooming
            QRect tepRect = r;
            tepRect.moveCenter(cr.center());
            paint.drawPixmap(tepRect.topLeft(), currentBackgroundImage);
        } else if (_backgroundMode == Tile) { // tile the image
            QPixmap scaled = currentBackgroundImage;
            if (minScale < 1.0) {
                scaled = scaled.scaled(scaled.size() * maxScale, Qt::KeepAspectRatio, Qt::SmoothTransformation);
            }
            for (int y = 0; y < cr.height(); y += scaled.height()) {
                for (int x = 0; x < cr.width(); x += scaled.width()) {
    ...
    for (auto rect = regToDraw.begin(); rect != regToDraw.end(); rect++) {
        drawBackground(paint, *rect, _colorTable[DEFAULT_BACK_COLOR].color, true /* use opacity setting */);
...
#else
        if (ev->pos().x() > dragInfo.start.x() + distance ||
                ev->pos().x() < dragInfo.start.x() - distance ||
                ev->pos().y() > dragInfo.start.y() + distance ||
                ev->pos().y() < dragInfo.start.y() - distance) {
            // we've left the drag square, we can start a real drag operation now
            emit isBusySelecting(false); // Ok.. we can breath again.
